    ...
    useEffect(() => {
        ...
        async function fetchUserData() {
            try {
                ...
                const userInfoResponse = await axios.get('http://localhost:3600/api/users/' + userId);
                setUserInfo(userInfoResponse.data);
                // Fetch upcoming exams (replace with your actual API endpoint)
                const examsResponse = await axios.post('http://localhost:3600/api/mock/exams/getExamsBasedOnTags', {
